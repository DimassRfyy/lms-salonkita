<x-layout>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 py-10" x-data="{
        ...
        <section
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div class="max-w-2xl space-y-2">
                    <span
                        class="inline-flex items-center gap-1.5 rounded-full bg-white/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-white backdrop-blur-xs">
                    ...
                    <p class="text-xs md:text-sm text-white/90 leading-relaxed">
                        Ajukan permohonan bimbingan ke mentor pilihan Anda untuk kelas <strong
                            class="text-white font-bold">{{ $activeEntitlement?->course?->name ?? 'Anda' }}</strong>.
                        Mentor akan me-review pengajuan sebelum jadwal dipilih.
                    </p>
                </div>
                    ...
                    <div class="flex items-start gap-3.5">
                        ...
                        <div>
                            <p class="font-bold text-base">Permohonan Anda Sedang Ditinjau oleh
                                {{ $currentActiveReq->mentor?->name ?? 'Mentor' }}</p>
        ...
        <div class="rounded-3xl border border-pink-100 bg-white p-6 md:p-8 shadow-sm">
            <div
                class="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between pb-4 border-b border-gray-100">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Daftar Mentor Tersedia</h2>
                    <p class="text-xs text-gray-500 mt-0.5">Pilih mentor yang sesuai dengan preferensi dan keahlian yang
                        kamu butuhkan.</p>
                </div>
                <a href="{{ route('mentoring.index') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @forelse($mentorCards as $mentor)
                    <div
                        class="group flex flex-col justify-between rounded-3xl border border-gray-100 bg-gradient-to-b from-white to-gray-50/50 p-6 transition duration-200 hover:border-pink-200 hover:shadow-md">
                        <!-- Top: Profile Biodata -->
                        <div>
                            <div class="flex items-start gap-4">
                                <div
                                    class="w-16 h-16 rounded-2xl overflow-hidden bg-pink-100 flex items-center justify-center text-pink-700 font-bold text-xl shrink-0 shadow-xs border border-pink-100">
                                    @if($mentor->avatar)
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center justify-between gap-2">
                                        <h3
                                            class="text-lg font-bold text-gray-900 group-hover:text-pink-600 transition truncate">
                                            {{ $mentor->name }}
                                        </h3>
                                        <span
                                    ...
                                    <p class="text-xs font-semibold text-pink-600 mt-0.5">
                                        {{ $mentor->job_title ?? 'Mentor Profesional Beauty' }}</p>
                                    @if($mentor->city)
                            ...
                            @if($mentor->bio)
                                <div
                                    class="mt-4 rounded-2xl bg-gray-50/80 p-3.5 border border-gray-100 text-xs text-gray-600 leading-relaxed line-clamp-3">
                                    {{ $mentor->bio }}
                                </div>
                            @else
                ...
                @empty
                    <div
                        class="md:col-span-2 rounded-3xl border border-dashed border-gray-200 bg-gray-50 p-12 text-gray-500 text-center">
