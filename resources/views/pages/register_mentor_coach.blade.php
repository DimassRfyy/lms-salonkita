                ...
                @unless($isAuthenticated)
                    ...
                    <div class="p-1 bg-gray-100/90 rounded-2xl flex items-center gap-1 mb-6 border border-gray-200/50">
                        ...
                        <a href="{{ route('register.mentor-coach', ['role' => 'mentor']) }}"
                            class="flex-1 py-2 px-2.5 rounded-xl text-center text-xs font-bold transition flex items-center justify-center gap-1.5 {{ !$isCoach ? 'bg-white text-pink-600 shadow-sm border border-pink-100' : 'text-gray-500 hover:text-pink-600 hover:bg-white/70' }}">
                            <svg class="w-3.5 h-3.5 {{ !$isCoach ? 'text-pink-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
